...
{    // Arrays of realistic data for sale items with actual images
    ...
    public function run(): void
    {
        ...
        // Clear references from related tables
        DB::table('bids')->delete();
        DB::table('favorites')->where('advertisement_id', '!=', null)->delete();
        DB::table('cart_items')->where('advertisement_id', '!=', null)->delete();
        
        // Now safe to delete advertisements
        DB::table('advertisements')->delete();
        $this->command->info('Cleared existing advertisements and related data.');
