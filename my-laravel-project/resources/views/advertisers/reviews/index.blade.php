...
@section('title', __('general.advertiser_reviews') . ' - ' . $user->name)

@section('content')
<div class="container py-4">
    <div class="card shadow-sm">
        <div class="card-header bg-light d-flex justify-content-between align-items-center">
            <h1 class="h3 mb-0">{{ __('general.advertiser_reviews') }} - {{ $user->name }}</h1>
            @auth
                @if(Auth::id() !== $user->id)
                    <a href="{{ route('advertiser.reviews.create', $user) }}" class="btn btn-primary">
            ...
            @if($reviews->count() > 0)
                <div class="rating-summary mb-4">
                    <div class="d-flex align-items-center">
                        <div class="h1 mb-0 me-3">{{ number_format($averageRating, 1) }}</div>
                        <div>
                            @for($i = 1; $i <= 5; $i++)
    ...
    <div class="mt-3">
        <a href="{{ route('advertisers.show', $user) }}" class="btn btn-secondary">
